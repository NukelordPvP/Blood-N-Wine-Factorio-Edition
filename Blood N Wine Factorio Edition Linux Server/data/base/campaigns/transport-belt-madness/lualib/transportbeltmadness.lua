...
function transport_belt_madness_init_level(this)
  ...
  for index, item in pairs(level.recipes) do
    global.allowed_items[item] = true
    recipeList[item].enabled = true
    game.players[1].insert(item)
    game.players[1].set_quick_bar_slot(quickBarIndex, item)
    quickBarIndex = quickBarIndex + 1
  end
  mod_gui.get_button_flow(game.players[1]).add{type="button", name="start", caption={"start"}, style = mod_gui.button_style}
  mod_gui.get_button_flow(game.players[1]).add{type="button", name="clear_button", caption={"clear"}, style = mod_gui.button_style}
...
function  transport_belt_madness_start_level(this)
  map_save()
  mod_gui.get_button_flow(game.players[1]).start.destroy()
  mod_gui.get_button_flow(game.players[1]).clear_button.destroy()
  mod_gui.get_button_flow(game.players[1]).add{type="button", name="back", caption={"back"}, style = mod_gui.button_style}
...
function map_load()
  ...
  if mod_gui.get_button_flow(game.players[1]).back then
    mod_gui.get_button_flow(game.players[1]).back.destroy()
  end
...
function check_built_items(event)
  local allowed_items = global.allowed_items or {}
  if event.name == defines.events.on_built_entity then
    local entity = event.created_entity
    if not entity.valid then return end
    if not allowed_items[entity.name] then
      entity.destroy()
    else
      entity.active = false
    end      
  ...
  if event.name == defines.events.on_player_cursor_stack_changed then
    ...
    if not allowed_items[stack.name] then
      stack.clear()
    end
  end
end
